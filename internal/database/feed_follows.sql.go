...
const getAllFeedFollowsFromUser = `-- name: GetAllFeedFollowsFromUser :many
SELECT id, feed_id, user_id, created_at, updated_at FROM feed_follows WHERE user_id = $1
`

func (q *Queries) GetAllFeedFollowsFromUser(ctx context.Context, userID uuid.UUID) ([]FeedFollow, error) {
	rows, err := q.db.QueryContext(ctx, getAllFeedFollowsFromUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
